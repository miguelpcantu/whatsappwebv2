...
window.WAPI.sendMessageToID = function (id, message, done) {
  ...
  try {
    // Nova versÃƒÂ£o do WhatsApp Beta
    if (WAPI.isMultiDeviceVersion()) {
      ...
      WAPI.getChat(id, chat => {
        if (chat) {
          ...
          console.log('Mensagem NÃƒÆ’O enviada!');
          done(false);
          return false;
        }
      });
    } else {
      // VersÃƒÂ£o antiga do WhatsApp  para um sÃƒÂ³ navegador
      try {
        ...
        // Quando ainda nÃƒÂ£o ÃƒÂ© um contato
...
window.WAPI.findJidFromNumber = (number) => {
  if (WAPI.isMultiDeviceVersion()) {
    return new Promise(resolve => {
      // Multiversion versÃƒÂ£o antiga
      Store.QueryExist.queryExist(WAPI.tryFixNumber(number)).then(value => {
        resolve( {
          status: 200,
          jid: value.wid
        })
      }).catch(() => {
        // Nova versÃƒÂ£o a partir de 07/07/2022
...
window.WAPI.sendImage = function (imgBase64, chatid, filename, caption, done) {
  ...
  return Store.Chat.find(idUser).then((chat) => {
    var mediaBlob = window.WAPI.base64ImageToFile(imgBase64, _filename);
